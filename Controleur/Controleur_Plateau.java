...
public class Controleur_Plateau implements MouseListener{
    ...
    void selectionPingouin(Point p){
        //Voir comment recup indice du pingouin + verifiÃ© si joueur a cliquer sur un pgn, sinon ne rien faire
        int i;
    ...
    void posePingouinInitialisation(Point p){
        System.out.println("il t a " + this.mp.getJoueurs().get(joueurcourant).getNbPingouinPose() + " de posé");
        if (this.mp.getJoueurs().get(joueurcourant).getNbPingouinPose() == 0 || 
        		this.mp.getJoueurs().get(joueurcourant).estPingouin(p) == -1)
